package com.clinicalsystem.data.remote.dto
...
@Serializable data class RegisterRequestDto(val firebaseIdToken: String, val email: String, val phone: String? = null, val role: String = "PATIENT")
...
fun DoctorDto.toDomain() = com.clinicalsystem.domain.model.Doctor(id = id, fullName = fullName, specialty = specialty, consultationFee = consultationFee ?: "0", rating = avgRating?.toDoubleOrNull() ?: 0.0, reviewCount = totalReviews, profileImage = profileImage, available = available)
fun AppointmentDto.toDomain() = com.clinicalsystem.domain.model.Appointment(id = id, patientId = patientId, doctorId = doctorId, slotId = slotId, date = slotDate ?: "", time = "$slotStartTime - $slotEndTime", status = status, type = type)
fun AuthResponseDto.toDomain() = com.clinicalsystem.domain.model.AuthResult(accessToken = accessToken, refreshToken = refreshToken, user = user.toDomain())
fun UserInfoDto.toDomain() = com.clinicalsystem.domain.model.User(id = id, email = email, role = role)
fun PaymentDto.toDomain() = com.clinicalsystem.domain.model.Payment(id = id, appointmentId = appointmentId, amount = amount, status = status)
fun ProfileDto.toDomain() = com.clinicalsystem.domain.model.UserProfile(fullName = fullName ?: "", dob = dob ?: "", bloodType = bloodType ?: "", gender = gender ?: "", profileImage = profileImage)
fun NotificationDto.toDomain() = com.clinicalsystem.domain.model.Notification(id = id, title = title, body = body ?: "", type = type, isRead = read, time = sentAt ?: "")
fun ChatMessageDto.toDomain() = com.clinicalsystem.domain.model.ChatMessage(id = id ?: "", senderId = senderId, content = content, time = sentAt ?: "", isMe = false)
